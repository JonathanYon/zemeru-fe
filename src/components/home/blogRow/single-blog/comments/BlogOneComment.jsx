...
const BlogOneComment = ({ match, comment, history }) => {
  ...
  const lyricsLikes = useSelector((state) => state.liked.blog);
  ...
  const deleteComment = async (commentID) => {
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_URL}/blogs/post/${id}/comments/${commentID}`,
        {
          headers: {
            ...
            <div className="d-flex justify-content-between">
              ...
              <div className="d-flex flex-row align-items-center">
                {lyricsLikes.includes(comment._id) ? (
                  <FaThumbsUp
                    onClick={() => dispatch(toggleLikeBlog(comment._id))}
                  />
                ) : (
                  <FaRegThumbsUp
                    onClick={() => dispatch(toggleLikeBlog(comment._id))}
...
export default withRouter(BlogOneComment);
